...
<div class="flex flex-col lg:flex-row mt-4 lg:mt-32 lg:h-screen">
    <!-- サイドバー -->
    <aside class="w-full lg:w-1/6 bg-gray-100 p-4 overflow-y-auto relative lg:fixed left-0 top-0 bottom-0 z-10">
        <div class="lg:py-32">
            <!-- 監査ラベル一覧 -->
            <ul class="mb-8">
            ...
            <!-- タグ一覧 -->
            <h3 class="text-m font-bold text-gray-400 mb-2">Source</h3>
            <ul>
                <li><a href="{{ route('rss.index') }}" class="text-gray-400 text-sm hover:underline mt-2">ALL</a></li>
                @foreach ($tags as $tag)
                <li>
    ...
    <!-- メイン -->
    <main class="w-full lg:w-5/6 p-4 overflow-y-auto mt-4 lg:mt-0 lg:ml-auto">
        ...
        <form id="main-summary-form" action="{{ route('article.summarize') }}" method="POST" class="mt-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="border p-4 rounded-3xl shadow bg-white">
                    ...
                    <p class="text-gray-400 text-sm">Source：{{ $item->tag_name }}</p>
                    <p class="text-sm text-gray-400">Date：{{ \Carbon\Carbon::parse($item->article_date)->format('Y-m-d H:i') }}</p>
                    <p class="text-gray-400 text-sm break-words whitespace-pre-wrap mt-2">Summary：<br>{{ $item->article_summary }}</p>
                </div>
                @endforeach
            </div>
